...
//enum Properties https://gist.github.com/smkplus/2a5899bf415e2b6bf6a59726bb1ae2ec
	Properties {
		...
		_SpecControlTintTex ("Specular Tint (RGB)", 2D) = "white" {}
		_SpecControlTex ("Spec & Gloss Map (RG)", 2D) = "white" {}
...
//		_NormalStrength ("Normal Strength", float) = 1
        ...
        //[Enum(UnityEngine.Rendering.ColorWriteMask)] __ColorWriteMask("ColorWriteMask", Float) = 15 //"All"
		[Space]
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surf BlinnPhong fullforwardshadows keepalpha alphatest:_AlphaCut addshadow
		...
		#pragma shader_feature SPEC_CONTROL_MAPS_ENABLED 
		...
		#ifdef SPEC_CONTROL_MAPS_ENABLED
		sampler2D _SpecControlTintTex;
		sampler2D _SpecControlTex;
		#endif//def SPEC_CONTROL_MAPS_ENABLED
		...
		struct Input {
			...
			#endif
			#ifdef SPEC_CONTROL_MAPS_ENABLED
			float2 uv_SpecControlTintTex;
			float2 uv_SpecControlTex;
			#endif//SPEC_CONTROL_MAPS_ENABLED
		};
...
//			}
			...
			#ifdef SPEC_CONTROL_MAPS_ENABLED
			fixed3 specTint = tex2D(_SpecControlTintTex,IN.uv_SpecControlTintTex).rgb;
			float2 specCont = tex2D(_SpecControlTex,IN.uv_SpecControlTex).rg;
			_SpecColor.rgb = _SpecColor.rgb * specTint;
			//N.B. 0 in o.Specular looks like a bug
			o.Specular *= specCont.r;
			o.Gloss *= specCont.g;
			#endif//SPEC_CONTROL_MAPS_ENABLED
		}
		ENDCG
	...
	FallBack "Diffuse"
	CustomEditor "BlinnPhongGeneralInspector"
}
